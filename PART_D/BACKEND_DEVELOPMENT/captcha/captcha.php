<?php
    ...
    $total_char = 4; // total char

    $letter = 'bcdfghjkmnpqrstvwxyz23456789';

    $random_captcha_dots  = 3; // noise: dots
    $random_captcha_line  = 3; // noise: lines

    $count = 0;

    while($count < $total_char) {
        $captcha_code .= substr(
            $letter, mt_rand(0, strlen($letter) - 1), 1
        );
        $count++;
    }
    ...
    for($count = 0; $count < $random_captcha_line; $count++) {
        imageline(
            $captcha_image,
            mt_rand(0,$captcha_image_width),
            mt_rand(0,$captcha_image_height),
    ...
    $text_box = imagettfbbox(
        $captcha_font_size, 0, $captcha_font, $captcha_code
    );
